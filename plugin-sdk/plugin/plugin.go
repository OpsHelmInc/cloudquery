...
type NewClientFunc func(context.Context, zerolog.Logger, []byte, NewClientOptions, *aws.Config) (Client, error)
...
type Plugin struct {
	...
	// whether to force a specific aws config
	overrideConfig *aws.Config
}
...
func (p *Plugin) Init(ctx context.Context, spec []byte, options NewClientOptions) error {
	...
	p.client, err = p.newClient(ctx, clientLogger, spec, options, p.overrideConfig)
	if err != nil {
