...
func GetNewClient(options ...Option) plugin.NewClientFunc {
	...
	return func(context.Context, zerolog.Logger, []byte, plugin.NewClientOptions, *aws.Config) (plugin.Client, error) {
		return c, nil
	}
}

func NewMemDBClient(ctx context.Context, l zerolog.Logger, spec []byte, options plugin.NewClientOptions, overrideConfig *aws.Config) (plugin.Client, error) {
	return GetNewClient()(ctx, l, spec, options, overrideConfig)
}

func NewMemDBClientErrOnNew(context.Context, zerolog.Logger, []byte, plugin.NewClientOptions, *aws.Config) (plugin.Client, error) {
...
func (c *client) Sync(_ context.Context, options plugin.SyncOptions, res chan<- message.SyncMessage, _ chan<- *schema.Resource) error {
	c.memoryDBLock.RLock()
